...
export class MockAPI {
  ...
  startLiveFeed(callback: (entry: LiveFeedEntry) => void): void {
    ...
    const usernames = [
      'DiceMaster99', 'HighRoller', 'LuckyStrike', 'GoldenBet', 'DiceKing',
      'BetHigh', 'WinStreak', 'CasinoPro', 'RollerX', 'DiceQueen',
      'LuckyDice', 'HighBet', 'CasinoKing', 'DiceLord', 'BetMaster'
    ]
   ...
   */
  async claimFaucet(): Promise<{ success: boolean; amount: number; message: string }> {
   ...
   * Verify roll hash
   */
  verifyRoll(serverSeed: string, clientSeed: string, nonce: number, hash: string): boolean {
