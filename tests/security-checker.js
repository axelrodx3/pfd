...
class SecurityChecker {
  ...
  async checkCertificate(hostname) {
    return new Promise((resolve) => {
      ...
      const req = https.request(options, (res) => {
        ...
        if (cert && cert.subject) {
          ...
          if (daysUntilExpiry > 30) {
            this.results.passed.push(`✅ SSL certificate valid for ${daysUntilExpiry} days`);
          } else if (daysUntilExpiry > 0) {
            this.results.warnings.push(`⚠️ SSL certificate expires in ${daysUntilExpiry} days`);
          } else {
      ...
      req.on('error', (error) => {
        this.results.failed.push(`❌ Certificate check failed: ${error.message}`);
        resolve();
      });
  ...
  async checkSecurityHeaders() {
    try {
      ...
      this.results.failed.push(`❌ Security headers check failed: ${error.message}`);
    }
  }
    ...
    switch (header) {
      case 'x-frame-options':
        if (value.toLowerCase() === 'deny' || value.toLowerCase() === 'sameorigin') {
          this.results.passed.push(`✅ X-Frame-Options set to secure value: ${value}`);
        } else {
          this.results.warnings.push(`⚠️ X-Frame-Options should be 'DENY' or 'SAMEORIGIN': ${value}`);
        }
        break;
      ...
      case 'x-content-type-options':
        if (value.toLowerCase() === 'nosniff') {
          ...
          this.results.warnings.push(`⚠️ X-Content-Type-Options should be 'nosniff': ${value}`);
        }
        break;
      ...
      case 'strict-transport-security':
        if (value.includes('max-age') && value.includes('includeSubDomains')) {
          ...
          this.results.warnings.push(`⚠️ HSTS should include max-age and includeSubDomains: ${value}`);
        }
        break;
    ...
    try {
      ...
      mixedContentPatterns.forEach((pattern, index) => {
        if (pattern.test(response)) {
          this.results.warnings.push(`⚠️ Potential mixed content detected (pattern ${index + 1})`);
        }
      });
      ...
      this.results.failed.push(`❌ Mixed content check failed: ${error.message}`);
    }
  }
    ...
    try {
      ...
      setCookieHeaders.forEach((cookie, index) => {
        ...
        if (cookieStr.includes('Secure')) {
          ...
          this.results.warnings.push(`⚠️ Cookie ${index + 1} missing Secure flag`);
        }
        ...
        if (cookieStr.includes('HttpOnly')) {
          ...
          this.results.warnings.push(`⚠️ Cookie ${index + 1} missing HttpOnly flag`);
        }

        // Check for SameSite
        if (cookieStr.includes('SameSite=Strict') || cookieStr.includes('SameSite=Lax')) {
          this.results.passed.push(`✅ Cookie ${index + 1} has SameSite attribute`);
        } else {
          this.results.warnings.push(`⚠️ Cookie ${index + 1} missing SameSite attribute`);
        }
      });
    } catch (error) {
...
if (require.main === module) {
  ...
  checker.checkSecurity().then(results => {
